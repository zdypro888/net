...
type Conn[T any] interface {
	...
	Read(ctx context.Context) (data T, err error)
...
type ConnHeart[T any] interface {
	// Heart 返回心跳消息的数据和下次心跳的时间点。
	// 首次心跳不要返回数据，只需返回下次心跳时间。
	Heart(connect bool, count uint64) (T, time.Time, bool)
}

// NotifyMessage 定义了可用于请求-响应匹配的消息接口
